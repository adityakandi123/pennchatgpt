...
import myMSALObj, { appScopes } from "../authProvider";
...
const GcpChatbot = ({ apiURL, hostingEnv }) => {
    ...
    const response = await fetch(
      "https://directline.botframework.com/v3/directline/conversations",
      {
        ...
        headers: {
          ...
          Authorization:
            "Bearer vaGZhBz9bWc.wdfBHyEl4p1fAld1k-N1JmL5fWzMRgYPlad64cxOk-w",
        },
      }
    );
  ...
  async function response(url, data) {
    ...
    if (!response.ok) {
      ...
      let errorresponses = {
        translated_response:
          "I'm sorry there was an error with the Bot Response, please try again",
        quick_replies: [],
        cards: [],
        urls: [],
      };
  ...
  async function getChatbotResponse(userInput) {
    ...
    if (userInput !== undefined) {
      let responseobj = {
        role: "assistant",
        content: resp?.translated_response,
      };
  ...
  const renderchatbotmessages = (message, index) => {
    ...
    if (
      ...
      message.content.translated_response =
        message.content.translated_response.replace(
          /\*\*(.*?)\*\*/g,
          "<strong>$1</strong>"
        );
      let modifiedContent = message?.content?.translated_response.replace(
        /\[([^\]]+)\]\(([^)]+)\)/g,
        function (match, $1, $2) {
          if ($1.includes("Doc")) {
            $1 = $1.replace("Doc", "");
            return (
              '<sup style="font-weight:bold;"><a href="' +
              $2 +
              '" target="_blank">[' +
              $1 +
              "]</a></sup>&nbsp;"
    ...
    if (
      ...
      message.content.translated_response =
        message.content.translated_response.replace(/\\n/g, "<br />");
      message.content.translated_response =
        message.content.translated_response.replace(/['"]+/g, "");
      ...
      if (matches) {
        ...
        elements.push(
          rendertranslatedresponsewithurls(
            modifiedContent,
        ...
        >
          <i
            className={`material-symbols-outlined ${
              ChatbotStyles["thumbs-icon"]
            } ${ChatbotStyles["thumbscolor"]} ${
              isThumbsUpSelected.includes(toggleindex)
                ? `${ChatbotStyles["thumbs-up-selected"]}`
                : ""
            }`}
            id="thumbupid"
            onClick={() => {
              handleThumbsUpClick(toggleindex);
            }}
          >
            thumb_up
          </i>
          <i
            className={`material-symbols-outlined ${
              ...
              isThumbsDownSelected.includes(toggleindex)
                ? `${ChatbotStyles["thumbs-down-selected"]}`
                : ""
            }`}
            id="thumbdownid"
            onClick={() => {
              handleThumbsDownClick(toggleindex);
            }}
          >
            thumb_down
          </i>
    ...
    return (
      <div className={ChatbotStyles["chatbot-message"]}>
        ...
        <div className={ChatbotStyles.dropdown}>
          <span
            className={ChatbotStyles["references-toggle"]}
            onClick={() => handleToggleClick(toggleindex)}
          >
            References
          </span>
          <div
            className={`references-collapse ${
              expandedIndexes.includes(toggleindex) ? "expanded" : ""
            }`}
          >
    ...
    return (
      ...
      >
        {message.map((ele, index) => (
          <button
            key={index}
            className={ChatbotStyles.quickbtn}
            onClick={() => handleQuickReplyClick(ele)}
            disabled={isLoading}
          >
            {ele}
          </button>
          ...
          {cards.map((card, index) => (
            <div key={index} className={ChatbotStyles.cardCard}>
              ...
              <div className={ChatbotStyles.cardtitle}>
                <span className={ChatbotStyles["font-weight-bold"]}>
                  {card.title}
                </span>
              ...
              {card.buttons.length > 0 &&
                card.buttons.map((button, buttonIndex) => (
                  <a
                    ...
                    target="_blank"
                    className={ChatbotStyles.cardlink}
                  >
                    {button.text}
                  </a>
                ))}
  ...
  const convertToWav = async (audioChunks) => {
    ...
    if (responseData.transcripts?.length > 0) {
      ...
      setMessages((prevMessages) => [...prevMessages, newMessage]);
      handleusermessage(
        responseData.transcripts[0],
        responseData.detected_language
      );
    }
  };
  ...
  const ChatLoadingAnimation = (type) => {
    const containerClass =
      type !== "user-audio" ? "loading-container" : "user-loading-container";
  ...
  const SendLoadingAnimation = () => {
    return (
      <div
        className={ChatbotStyles["send-loading-container"]}
        id="send-loading-container"
      >
        <div className={ChatbotStyles["bouncing-balls"]}>
          {[1, 2, 3].map((index) => (
  ...
  return (
    <div className="chatbot-component">
      ...
      {chatclick == true && (
        <div
          className={`${ChatbotStyles["chat-container"]}`}
          id="chat-container"
        >
          <div className={ChatbotStyles["chat-header"]} id="chat-header">
            ...
            <div className={ChatbotStyles["chat-buttons"]}>
              <div className={ChatbotStyles["settings-div"]} id="settingsdiv">
                ...
                {showsetting && (
                  <div className={ChatbotStyles["settings-options"]}>
                    <div className={ChatbotStyles["contrast-option"]}>
                      <label
                        htmlFor="contrast-switch"
                        className={ChatbotStyles["contrast-switch"]}
                      >
                        Contrast
                      </label>
                    ...
                    <div className={ChatbotStyles["font-size-options"]}>
                      <label
                        htmlFor="font-size"
                        className={ChatbotStyles["font-size"]}
                      >
                        Font Size
                      </label>
              ...
              <div
                className={ChatbotStyles["reset-chat"]}
                id="reset-chat"
                onClick={resetChat}
              >
          ...
          <div className={ChatbotStyles["chat-input"]}>
            ...
            {isLoading ? (
              <div className={ChatbotStyles.send} id="senddiv">
                {SendLoadingAnimation()}
              </div>
            ) : (
              <div className={ChatbotStyles.send} id="senddiv">
                {txtInputValue.length == 0 ? (
                  <span
                    className="material-symbols-rounded"
                    id="micicon"
          ...
          <div className={ChatbotStyles["footer-msg"]}>
            ...
            <span>
              Verification of critical info from AI generated responses is
              recommended
            </span>
          </div>
        </div>
